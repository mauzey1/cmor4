...
Current CMOR4 architecture (from `src/cmor4/core.py`):
...
- **`src/cmor4/_writer_utils.py`**: Helper functions for Zarr operations, chunk iteration, time validation
...
- **`src/cmor4/__init__.py`**: Export `DatasetWriter`
...
1. **Core `DatasetWriter` with Zarr staging** (`src/cmor4/writer.py`):
   - `__init__`: validation, Zarr store creation, metadata initialization
   - `write()`: chunk appending, validation
   - `close()`: NetCDF finalization via direct netCDF4 write

2. **Basic tests** (`tests/test_incremental_writes.py`):
   - Single write, multiple writes, shape validation, time monotonicity

3. **Append mode** (extend `DatasetWriter.close()`):
   - File compatibility checking, concatenation, atomic replacement

4. **Append mode tests** (`tests/test_append_mode.py`)

5. **Preserve mode** (extend `DatasetWriter.close()` and internal state):
   - Metadata retention, Zarr store reset

6. **Zfactor support** (extend `DatasetWriter.write()`):
   - Per-chunk zfactor data, validation

7. **Integration tests** (`tests/test_cmip7_examples.py`, `tests/test_large_datasets.py`)

8. **Documentation** (user guide, API reference, examples)
...
3. Append mode successfully extends existing CMOR4-generated files
4. Memory usage remains bounded regardless of total dataset size
5. All validation rules (axis, variable, global attributes, CMIP7 chunking) apply correctly
6. Test coverage ≥ 90% for new code
7. Documentation includes clear usage examples and migration guidance
