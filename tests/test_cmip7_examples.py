...
_LON_BNDS = np.array(
    [[-45.0, 45.0], [45.0, 135.0], [135.0, 225.0], [225.0, 315.0]], dtype="d"
)
...
class TestExample04AuxiliaryCoordinates(unittest.TestCase):
    ...
    def test_variable_cell_methods(self):
        self.assertEqual(
            ...
            "depth: longitude: sum where sea "
            "(along a zig-zag grid path spanning a basin)  time: mean",
        )
...
class TestExample06ComplexGrid(unittest.TestCase):
    ...
    def setUp(self):
        ...
        y_axis = project.axis(
            ...
            values=self._Y_VALS,
            bounds=np.array(
                [[-5000.0, 5000.0], [5000.0, 15000.0], [15000.0, 25000.0]], dtype="d"
            ),
            units="m",
        )
        x_axis = project.axis(
